...
class aDroneIF(multiprocessing.Process):
    ...
    def __init__(self, log_l, base_ip, init_ip, base_ctrl_port, base_sens_port, id, ns, d_e_l, d1, d2, action_list, stop_action, group = None, target = None, name = "aDrone", args = (), kwargs = {}):
        ...
        logging.debug('[INFO] %s.%s init with ip: %s:%s|%s' % (self.name, self.myid, self.myip, self.my_control_port, self.my_sens_port))
    ...
    def run(self):
        ...
        while self.ns.running:  # implement drone management
            ...
            logging.info("aDroneIF dictionary: ", self.action[self.myid])

            # 2) Perform pending action --------------------------------------------------------------------------------
            result = perform_action(self.action[self.myid], self.vehicle, self.stop_action, self.myid, self.log_l, self.action[self.myid]['wait'])
            ...
            self.sens_dict['latitude'] = self.vehicle.location.global_relative_frame.lat
            self.sens_dict['longitude'] = self.vehicle.location.global_relative_frame.lon
            self.sens_dict['altitude'] = self.vehicle.location.global_relative_frame.alt
            self.sens_dict['battery'] = self.vehicle.battery.level
            logging.info("Battery level drone" + str(self.myid) + ": " + str(self.d2[self.myid]['battery']))

            logging.info("temperature: " + str(self.d1[self.myid]['temperature']) + " altitude: " + str(self.d2[self.myid]['altitude']))
            ...
            logging.debug("[DEBUG] %s.%s - d1:%s d2:%s de_l:%s" % (self.name, self.myid, self.d1[self.myid], self.d2[self.myid], self.de_l[self.myid]))
            # ----------------------------------------------------------------------------------------------------------
        swarm_logger.info('%s.%s closing', self.name, self.myid)
        logging.debug("[INFO] %s.%s closing" % (self.name, self.myid))
        return
...
def perform_action(action, vehicle, stop_action, _id, log = logging.INFO, wait_time = 0):
    ...
    if action['action'] == "take-off":
        drone_lib.arm_and_takeoff(action['altitude'], vehicle, stop_action, _id, log)
        time.sleep(take_off_wait_time[_id])
        ...
        print(float(action['condition_yaw']))
        drone_lib.condition_yaw(float(action['condition_yaw']), vehicle, stop_action, _id)
        time.sleep(3);
        drone_lib.send_ned_velocity(-1, 0, 0, 2, vehicle, stop_action, _id, log)
        if stop_action[_id] is False:
            temp = action
            temp['action'] = "action complete"
        else:
