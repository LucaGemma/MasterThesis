...
#from swarmGlobals import uav_n, ctrl_l, fname, log_l, ver, base_ip, init_ip, base_ctrl_port, base_sens_port, valid_pattern, detect_task, drone_missions_directory, min_alt
...
class swarmManager(multiprocessing.Process):
    ...
    def __init__(self, log_l, uav_n, ns, d_l, d_e_l, d1, d2, action_list, stop_action, TEMP_LIMIT, BATT_LIMIT, group = None, target = None, name = "swarmManager", args = (), kwargs = {}):
    ...
    def run(self):
        ...
        #counter used to wait some time above temperature limit before calling RTL
        count = []
        for i in range(self.uav_n):
            count.append(0)
            self.yaw.append(0)
        ...
        for i in range(self.uav_n):
            for line in open(drone_missions_directory + '/drone_[' + str(i) + ']_actions.txt', 'r'):
                ...
                if current_action[1] == 'load_path':
                    firstline = True
                    (latitude, longitude, altitude_, condition_yaw_) = drone_lib.load_path(str(current_action[2]))
                    for z in range(len(latitude)):
                        # discard first value, for it is the home location chosen when the waypoints file was created
                        if z == 0:
                            continue
                        if float(altitude_[z]) < min_alt:
                            altitude_[z] = altitude
                        s = 'drone' + str(i) + ',' + 'go-to,' + str(altitude_[z]) + ',' + str(latitude[z]) + ',' + str(longitude[z]) + ',' + str(0) + ',' + str(0) + ',' + str(0) + ',' + str(condition_yaw_[z]) + ',' + str(waypoints_wait_time[i])
        ...
        while self.ns.running:
            ...
            if self.ns.CURRENT_TASK != '':
                if self.ns.CURRENT_TASK == 'land' or self.ns.CURRENT_TASK == 'rtl':
                    for i in range(self.uav_n):
            ...
            # 3) Update actions for every UAV --------------------------------------------------------------------------
            for i in range(self.uav_n):
                if self.d1[i]['temperature'] >= self.TEMP_LIMIT:
                    count[i] += 1
                if count[i] >= 150 and self.stop_action[i] == False:
                    print("WARNING RTL BECAUSE TEMPERATURE IS TOO HIGH: {}".format(self.d1[i]['temperature']))
                    #print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA {}".format(count))
                    self.stop_action[i] = True
                    #self.ns.CURRENT_TASK = 'rtl'
                    self.ns.CURRENT_TASK = 'drone' + str(i) + ',' + 'move-forward,' + str(self.d2[i]['altitude']) + ',' + ' ' + ',' + ' ' + ',' + str(5) + ',' + ' ' +',' + ' ' + ',' + str(self.yaw[i])
                    print(self.d2[i]['altitude'])
                    #self.drones_actions_list[i].insert(0,s)
                    #print(self.drones_actions_list[i])
                    count[i] = 0
                if self.d2[i]['battery'] <= self.BATT_LIMIT:
                    count[i] += 1
                if count[i] >= 150 and self.stop_action[i] == False:
                    print("WARNING RTL BEACUSE BATTERY VOLTAGE IS TOO LOW: {}".format(self.d2[i]['battery']))
                    self.stop_action[i] = True
                    self.ns.CURRENT_TASK = 'rtl'
                    count[i] += 1
    ...
    def update_action(self, i, kb_command = False, pending_action = False):
        ...
        if kb_command:
            if self.ns.CURRENT_TASK != '':
                ...
                if self.ns.CURRENT_TASK == 'land' or self.ns.CURRENT_TASK == 'rtl':
                    #print('current task: {}'.format(self.ns.CURRENT_TASK))
                    temp0 = self.action[i]
                    temp0['action'] = self.ns.CURRENT_TASK
                    self.action[i] = temp0
                else:
                    print('ENTERED IN UPDATE_ACTION WITH KEYBOARD INTERRUPT')
                    self.de_l[i] = 0
                    ...
                    temp = self.action[i]
                    print(temp)
                    print('current task: ' + str(current_task[0]) + ' , ' + str(current_task[1])+ ' , ' + str(current_task[8]))
                    ...
                    while k < len(current_task):
                        if k == 2:
                            ...
                            temp['condition_yaw'] = current_task[8]
                        k = k + 1
                        print(temp)
                    self.action[i] = temp
                    self.ns.CURRENT_TASK = ''
            ...
            if pending_action:
                ...
                while k < len(_current_action):
                    if k == 2:
                        ...
                        temp['condition_yaw'] = _current_action[8]
                        self.yaw[i] = temp['condition_yaw']
                    elif k == 9:
                        temp['wait'] = float(_current_action[9])
                    k = k + 1
                self.action[i] = temp
            swarm_logger.debug('[%s] drone:%s d1[0]:%s d2:%s', self.name, i, self.d1[0], self.d2[i])
            if self.log_l == logging.DEBUG:
                print "[DEBUG][%s] drone:%s d1[0]:%s d2:%s" % (self.name, i, self.d1[0], self.d2[i])
